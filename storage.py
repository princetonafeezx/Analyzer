"""Shared file helpers for local data and reports.

I kept this module pretty small on purpose. The feature modules do most of the
interesting work, and this file just makes it easier for the app to save and
reload data between commands.

JSON, CSV, and text reports are written via a temporary file in the same
...
def get_data_dir(base_dir: str | Path | None = None) -> Path:
    ...
    If ``base_dir`` is omitted, uses the ``ANALYZER_DATA_DIR`` environment
    variable when set; otherwise ``./analyzer_data`` under the current
    working directory.
    """
    if base_dir is not None:
        root = Path(base_dir)
    else:
        env = (os.environ.get("ANALYZER_DATA_DIR") or "").strip()
        if env:
            root = Path(env).expanduser()
        else:
            root = Path.cwd() / "analyzer_data"
...
def get_report_path(base_dir: str | Path | None = None) -> Path:
    return get_data_dir(base_dir) / "analyzer_report.txt"
...
def load_categorized_transactions(path: str | Path | None = None) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    the same parsing rules as :func:`parsing.parse_amount`.
    """
